...
#include <time.h>
#define LIFE 2 //寿命
#define NEW_LIFE 70 //草食動物が生まれる確率
#define DEATH 20 //突然死の確率
...
void grow_grass(int h,int w,const int height,const int width,int cell[height][width]);
...
void my_update_cells(const int height,const int width,int cell[height][width]){
    ...
    for(i=0;i<height;i++){
        for(j=0;j<width;j++){
            if(copy[i][j]==10){//草の場合
                grow_grass(i,j-1,height,width,cell);
                grow_grass(i,j+1,height,width,cell);
                grow_grass(i-1,j,height,width,cell);
                grow_grass(i+1,j,height,width,cell);
...
//草を生やす
void grow_grass(int h,int w,const int height,const int width,int cell[height][width]){
    if(h>=0 && w>=0 && h<height && w<width && cell[h][w]==0){
        cell[h][w] = 10;
    }else if(h == -1 && w == -1 && cell[height-1][width-1]==0){
        cell[height-1][width-1] = 10;
    }else if(h == -1 && w == width && cell[height-1][0]==0){
        cell[height-1][0] = 10;
    }else if(h == height && w == -1 && cell[0][width-1]==0){
        cell[0][width-1] = 10;
    }else if(h == height && w == width && cell[0][0]==0){
        cell[0][0] = 10;
    }else if(h == -1 && cell[height-1][w]==0){
        cell[height-1][w] = 10;
    }else if(h == height && cell[0][w]==0){
        cell[0][w] = 10;
    }else if(w == -1 && cell[h][width-1]==0){
        cell[h][width-1] = 10;
    }else if(w == width && cell[h][0]==0){
        cell[h][0] = 10;
    }
}
