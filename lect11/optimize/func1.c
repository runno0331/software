...
{
  double E = 0.0;
  for(i = 0; i < sizeof(data)/Sample; i++){
    E += (data[i][2] - (x[0]+data[i][1] + x[1])) * (data[i][2] - (x[0]+data[i][1] + x[1]));
  }
...
void f_gradient(const double x[], double g[])
{
  g[0] = 0;
  g[1] = 0;
  for(i = 0; i < sizeof(data)/sizeof(Sample); i++){
    g[0] += -2 * (data[i][2] - x[0]*data[i][1] - x[1]) * data[i][1];
    g[0] += -2 * (data[i][2] - x[0]*data[i][1] - x[1]);
  }
}
