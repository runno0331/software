/*
焼きなまし法山登り法において繰り返しの初めのうちは経路が多少長くなっても採用する。改善された場合は必ず採用する。
遷移確率は現状の経路と変更後の経路の距離の差を比例係数Tで割ったもののexpを取ったものである。そのため改善されるときに
遷移確率は必ず1より大きくなる。また比例係数Tはループ一周ごとに減衰していき、悪い経路は採用されづらくなる。
*/
...
double cool(double T, double coolrate);
int change_route(double distance_tem, double distance_cur, double T);
...
// 遷移確率　遷移する時1を返し、遷移しないとき0を返す
int change_route(double distance_tem, double distance_cur, double T)
{
  if(0.0001 * (rand() % 10000) < exp(0.1*(distance_cur - distance_tem)/T)){ // 改善された場合必ず遷移する
    return 1;
  }else{
    return 0;
  }
  ...
  while(change == 1 || T < 0.0001){
      ...
      for(int i = 0; i < n; i++){
          for(int j = i+1; j < n; j++){
              ...
              if(change_route(distance_tem,distance_cur,T) == 0){   // 遷移する場合
                  switch_route(route,i,j);
              }else{
                  change = 1;
